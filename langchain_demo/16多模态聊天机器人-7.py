...
def transcribe_audio(audio_path):
    ...
    try:
        file_extension = os.path.splitext(audio_path)[1].lower().replace('.', '')
        if file_extension not in ['wav', 'mp3']:
            raise ValueError("OpenAI 聊天接口目前主要支持 wav 或 mp3 格式的音频输入。")
...
def submit_messages(history):
    ...
    if user_messages:
        for x in user_messages:
            ...
            elif isinstance(x['content'], list):  # Gradio 6.x 格式: [{'text': '...', 'type': 'text'}] 或 [{'file': {...}, 'type': 'file'}]
                for block in x['content']:
                    if block.get('type') == 'text' and block.get('text'):
                        content.append({'type': 'text', 'text': block['text']})
                    elif block.get('type') == 'file' and block.get('file'):
